...
### Query 4 — Potential RedKitten Persistence (Scheduled Task) (Sentinel)
Searches for scheduled task creation events that reference AppVStreamingUX.exe (persistence mechanism seen in the campaign).
...
// Look for the abused binary in the task definition
| where TaskXML has "AppVStreamingUX.exe"
// Optional: Filter for non-standard paths within the XML content if parsing allows,
// strictly searching for the binary name is a strong lead given its specific abuse here.
| parse TaskXML with * '<Command>' Command '</Command>' *
| where not(Command has "Program Files")
| project TimeGenerated, Computer, Account, Command, TaskXML
```
